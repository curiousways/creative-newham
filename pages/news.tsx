      ...
      >
        <Panel title="News">
          <div className="cn-text">
            <p>News &amp; events for Creative Newham members. You can also find out what we've been up to on <a href="https://www.instagram.com/creativenewham/">Instagram</a> and <a href="https://twitter.com/creativenewham/">Twitter</a>.</p>
          </div>
        </Panel>
      </motion.div>
          ...
          {posts.map((post) => (
            <div key={post.title} className="flex flex-col overflow-hidden rounded-lg shadow-lg">
              <div className="flex-shrink-0">
                <img className="h-48 w-full object-cover mix-blend-screen opacity-70 grayscale bg-cn-orange" src={post.imageUrl} alt="" />
              </div>
              <div className="flex flex-1 flex-col justify-between bg-white bg-opacity-10 p-6">
                <div className="flex-1">
                  <a href={post.href} className="mt-2 block no-underline hover:text-cn-orange">
                    <p className="text-2xl leading-tight">{post.title}</p>
                    <p className="mt-3 text-base">{post.description}</p>
                  </a>
                </div>
                <div className="mt-6">
                  <a className="hover:text-cn-orange" href={post.href}>View article</a>
                </div>
              </div>
            </div>
          ))}
